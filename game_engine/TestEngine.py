...
class EngineTestCases(unittest.TestCase):
    ...
    def testOnePlayer(self):
        ...
        assert raceInfo.Laps == 5, "Default lap count differs from 5"
        assert raceInfo.JoinedPlayers == 1, "Player count is not 1"

        self.sender.sendAsync("ControllerEvent", player="0", event=UD, data="-32767")
        time.sleep(0.25)
        raceInfo = self.receiver.getData("RaceInfo")
        assert raceInfo.Laps == 6, "Default lap count differs from 6"


        self.sender.sendAsync("ControllerEvent", player="0", event=DUP, data="1")
        time.sleep(0.25)
        raceInfo = self.receiver.getData("RaceInfo")
        assert raceInfo.Laps == 7, "Default lap count differs from 7"


        self.sender.sendAsync("ControllerEvent", player="0", event=DDOWN, data="1")
        time.sleep(0.25)
        raceInfo = self.receiver.getData("RaceInfo")
        assert raceInfo.Laps == 6, "Default lap count differs from 6"
        ...
        for lap in range(raceInfo.Laps - 1):
            # delay for lap times
            time.sleep(0.25)
